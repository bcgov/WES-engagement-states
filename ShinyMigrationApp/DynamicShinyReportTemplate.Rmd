...
CombinedStates1 <- readRDS("data/CombinedStates1.rds") # load data
...
kable(CombinedStates1 %>% filter(ORGID15==input$Organization), caption = "Table 2: Organizational Engagement State", digits = 2, 
...
{
CombinedStates1 %>% filter(ORGID15==input$Organization) %>% filter(EngagementScore==variable) %>% select(NetGainOrLoss)
}
