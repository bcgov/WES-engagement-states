...
params:
  Organization: 
    label: "Organization"
    value: Great Blue Heron
    input: select
<<<<<<< HEAD
    choices: [GBH, HUM, LOON, OOWL]
=======
    choices: [Great Blue Heron, Hummingbird, Loon, Owl]
>>>>>>> 121922792ddb67d36d710a4a235f3dfe3a1020fb
  wd:
...
dataFile <- file.path(params$wd, "data", "MockCombinedStates1.rds")
MockCombinedStates1 <- readRDS(dataFile) # load data

IndexOrgName <- MockCombinedStates1 %>% filter(ORGANIZATION15==params$Organization) %>% select(ORGANIZATION15) %>% distinct()
...
kable(MockCombinedStates1 %>% filter(ORGANIZATION15==params$Organization), caption = "Table 2: Organizational Engagement State", digits = 2, 
...
{
MockCombinedStates1 %>% filter(ORGANIZATION15==params$Organization) %>% filter(EngagementScore==variable) %>% select(NetGainOrLoss)
}
